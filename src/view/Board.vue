<template>
  <div>
    <header class="jumbotron mt-4">
      ...
      <p>게시글을 작성할 수 있습니다.</p>
      <p class="lead">
        <b-button variant="success" size="lg">글쓰기</b-button>
      </p>
    </header>
    <body>
      <b-row>
        ...
        <b-col lg="2" class="my-1">
          <b-button block="block" variant="primary" @click="handleSearch"
            >서버조회</b-button
          >
        </b-col>
      </b-row>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "subTitle", label: "부제목", sortable: true },
      ],
      items: [],
      totalRows: 0,
      perPage: 10,
  ...
  computed: {
    ...
    filterOptions() {
      return this.fields
        .filter((f) => f.sortable)
        .map((f) => {
          return { value: f.key, text: f.label };
        });
    },
  },
};
